...
def validate_prompt_body(body: str) -> List[ValidationIssue]:
    """Ensure prompt instructions are present."""
    if body.strip():
        return []
...
def validate_prompt_references(file_path: str, body: str, line_offset: int) -> List[ValidationIssue]:
    """Validate repo-style #file references used by tracked prompt files."""
    issues: List[ValidationIssue] = []
...
def _position_from_offset(content: str, offset: int) -> tuple[int, int]:
    """Return 1-based line and column information for an offset."""
