...
- Preserve body pose first, then reduce walking stride/yaw motion until the generated foot targets are IK-valid.
...
- If scale reaches zero and IK still fails, classify the body pose itself as infeasible and skip publishing with a clear warning.
...
- Body pose too aggressive: robot holds/restores previous motion state and logs that body pose is infeasible even with zero stride.
...
- Log accepted scale when limiting occurs, for example:
  `IK limiter scaled walking motion to 0.73 for gait=tripod phase=0.81`
...
- Unit-test limiter with a fake IK predicate:
  ...
  - returns failure when zero walking motion is still infeasible
...
## Assumptions
- Priority policy is body-first: preserve body translation/rotation/height before walking stride.
