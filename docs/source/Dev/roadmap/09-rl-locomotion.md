...
The extended goal: stop telling the robot *how* to walk and let it learn gaits that exploit its
...
- **A1-16 servos are position-controlled.** No torque interface, no current feedback. The policy
  ...
  blocker. Torque-feedback servos (XC430-T240BB-T class) later improve *observations* and enable
...
1. **Model conversion + system ID.** Export the URDF to the trainer's format; identify the servo
   ...
   is *the* sim-to-real lever for position-controlled robots.
2. **Training environment.** Isaac Lab (GPU, mature legged-robot tasks) or MuJoCo MJX are the
   sane choices in 2026; Gazebo is far too slow for training and stays as the *validation* gate.
...
results — hexapods are statically stable, which makes the task *easier* than quadrupeds. The
honest risks:
...
- **Sim-to-real gap in the feet**: rubber tips on hard floor vs sim friction — randomize wide,
  validate on multiple real surfaces early with a *conservative* policy before chasing speed.
