...
14. **Never mark a `@pytest.mark.launch(...)` (`launch_pytest`) test with `@pytest.mark.flaky` (`pytest-retry`).** `launch_pytest`'s `pytest_pyfunc_call` hookwrapper re-wraps `pyfuncitem.obj` on every `pytest_runtest_call`, guarded by an `_launch_pytest_wrapped` flag that ordinary single-function launch tests never set. `pytest-retry`'s retry path tears down fixtures, resets them, and re-invokes `pytest_runtest_call`, which re-wraps the already-wrapped test callable and ends up mixing two `asyncio` event loop instances (the stale, closed one from the first attempt plus a fresh one) — a hard failure, not a graceful retry. This applies to every launch test shape (plain function, coroutine, generator; any fixture scope), not just module-scoped ones. This was tried and reverted the same day, leaving stale "Retries via pytest-retry" comments in `drqp_moveit/test/test_demo_gazebo_launch_smoke.py` — don't trust those comments as evidence the mechanism is active; check the actual decorator/marker.
