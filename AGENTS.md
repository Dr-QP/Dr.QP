...
7. **Check build logs** in `log/latest_build/` if builds fail. In CI, `colcon` build and test use reduced (default) console output, so a GitHub Actions log only shows _which_ package failed, not the actual failure — download the colcon logs and test report artifacts (via `/agentdev:extract-github-actions-logs`) to see it
...
15. **Every apt package the Ansible roles install is version-pinned** in `docker/ros/ansible/roles/<role>/vars/apt_pins_<suite>_<arch>.yml`, one file per Ubuntu release and architecture (ROS debs carry a per-architecture build timestamp in their version string). Renovate bumps the pins; add or remove a package by editing every architecture's file and then running `scripts/apt-pins-refresh.py`. The repositories the script resolves against (`DEFAULT_REPOS`/`ROLE_REPOS`) must stay in step with the `registryUrls` in `.github/renovate.json`. Never add a blanket `apt upgrade` anywhere in these playbooks — it can only move a package off a version this repository records. Upgrading inherited packages belongs to the base image prep phase, which lives in agentdev; this repository consumes the result through the agent-desktop digest in `docker/ros/desktop/ros-desktop.Dockerfile`
