...
7. **Check build logs** in `log/latest_build/` if builds fail. In CI, `colcon` build and test use reduced (default) console output, so a GitHub Actions log only shows *which* package failed, not the actual failure — download the colcon logs and test report artifacts (via `/agentdev:extract-github-actions-logs`) to see it
