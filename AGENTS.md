...
14. **`@pytest.mark.flaky` on a `@pytest.mark.launch(...)` (`launch_pytest`) test requires `drqp_launch_testing.launch_pytest_retry`.** `launch_pytest` re-wraps `pyfuncitem.obj` in place on every call; a plain retry re-wraps the previous attempt's stale wrapper and crashes with a closed/duplicated `asyncio` event loop instead of retrying. Register the fix via `pytest_configure` in the package's `test/conftest.py` (call `launch_pytest_retry.pytest_configure(config)`; see `drqp_moveit/test/conftest.py` for the pattern) before adding `flaky` to any launch test — see `drqp_launch_testing/test/shutdown_behavior/SPEC.md` (combo 6) for the root cause and proof. Even patched, retry only usefully rescues **function-scoped** launch fixtures (combo 5): `pytest-retry` does not re-run a `module`-scoped fixture's setup on retry, so a combo 4 (module-scoped shared simulation) `shutdown=True` test is crash-safe but not relaunch-safe — don't add `flaky` there expecting it to help. `pytest-retry` also isn't pip-installable via a plain `pip install` in this devcontainer (PEP 668); it needs `--break-system-packages`, matching how `pytest-rerunfailures` etc. are already present system-wide.
